...
class Agent:
    ...
    def get_utility_model(self):
        return models.get_model(
            models.ModelType.CHAT,
            models.ModelProvider.OPENAI,  # Use OPENAI provider for OpenRouter compatibility
    ...
    async def call_chat_model(self, prompt, **kwargs):
        from python.helpers.print_style import PrintStyle
        import os
        from langchain_openai import ChatOpenAI
        api_key = os.getenv("API_KEY_OPENROUTER")
        if not api_key:
            PrintStyle.error("API_KEY_OPENROUTER not set in environment")
            raise ValueError("API_KEY_OPENROUTER not set")
        PrintStyle.info(f"Using API key: {api_key[:4]}...{api_key[-4:]}")
        model_name = os.getenv("chat_model_name", "deepseek/deepseek-r1:free")
        ...
        model = ChatOpenAI(
            ...
            default_headers={
                ...
                "X-Title": os.getenv("OPENROUTER_X_TITLE", "Agent Zero")
            }
        )
        response = ""
